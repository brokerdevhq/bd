(ns ca.brokerdev.core.web.core
  (:require
   ...
   [ca.brokerdev.core.web.lifecycle.load]
...
(defn start! [environment]
  ...
  (let [config (dissoc
                (config/read-config "web/config.edn" {:profile environment}))]
...
(defn -main [& args]
  (let [{:keys [options errors help]} (config/parse-opts args cli-options)]
    (cond
      help (do (println "Usage: web [options]") (System/exit 0))
...
(comment
  ...
  (config/read-config "web/config.edn" {:profile :test})
